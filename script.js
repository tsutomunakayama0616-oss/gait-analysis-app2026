...
function diagnoseGait(pR, pL, abdR, abdL, addR, addL, speed) {
  ...
  if (pR > 10 || pL > 10)
    types.push("骨盤が左右に揺れやすい歩き方です。安定性を高める練習が役立ちます。");

  if (abdR < 5 || abdL < 5)
    types.push("足を横に広げる力が弱い可能性があります。おしりの横の筋トレが効果的です。");

  if (addR > 5 || addL > 5)
    types.push("足が内側に入りやすい歩き方です。バランス練習が役立ちます。");

  if (speed < 80)
    types.push("歩く速さがゆっくりめです。体力や筋力を少しずつ高めると良いでしょう。");
...
function recommendExercises(pR, pL, abdR, abdL, addR, addL, speed) {
  ...
  if (abdR < 5 || abdL < 5) ids.push(9, 10, 11);
  if (addR > 5 || addL > 5) ids.push(4, 14, 15);
  if (speed < 80) ids.push(16, 17);
